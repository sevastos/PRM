...
SerPro.prototype.sendNow = function(data, doneResponse, doneSend) {
  debug('SerPro.sendNow', 'Data:', data);
...
SerPro.prototype._processReplyPart = function(data) {
  ...
  debug('[process:response:part]', 'String:', data.toString(), '[raw]:', data, '(Length', (''+data).length + ')');
  ...
  if (this.replyTimeout) {
    ...
    this.replyTimeoutTimer = setTimeout(
      this.replyTimeout,
      this.conventions.replyTimeoutDuration
    );
  }
...
SerPro.prototype._assembleReplyPart = function(data) {
  ...
  this.runningBuffs = this.runningBuffs.concat(data);
};
...
SerPro.prototype._isReplyEnding = function(data) {
  var str = String(data).toString();
  if (str.slice(-1) === this.conventions.responseDelimiter) {
    this.runningBuffs = Buffer.concat(this.runningBuffs);
    if (
      this.runningBuffs.length === 2 &&
      [0, 1].indexOf(this.runningBuffs[0]) !== -1 &&
      this.runningBuffs[1] === this.conventions.responseDelimiter.charCodeAt(0)
    ) {
      // Boolean
      this.runningReply = !!this.runningBuffs[0];
      debug('response:ending', 'Recognized boolean response');
    }
    return true;
  }
  return false;
